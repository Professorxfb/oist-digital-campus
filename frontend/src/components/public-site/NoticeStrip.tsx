  ...
  return (
    <section className="border-b border-blue-100 bg-blue-50">
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-2 px-4 py-3 text-sm text-slate-700 sm:flex-row sm:items-center sm:px-6 lg:px-8">
        <span className="shrink-0 rounded-full bg-blue-700 px-3 py-1 text-xs font-semibold uppercase tracking-[0.12em] text-white">
          Notice
        </span>
        <a className="font-medium text-slate-900 hover:text-blue-800" href={`/notices/${notice.slug}`}>
          {notice.title}
        </a>
        {notice.published_at ? (
